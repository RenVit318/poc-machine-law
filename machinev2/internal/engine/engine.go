...
import (
	...
	contexter "github.com/minbzk/poc-machine-law/machinev2/internal/context"
...
func NewRulesEngine(spec map[string]any, serviceProvider contexter.ServiceProvider, referenceDate string) *RulesEngine {
...
func (re *RulesEngine) Evaluate(
	...
	if bsn, ok := parameters["BSN"].(string); ok {
		...
		if provider, ok := re.ServiceProvider.(contexter.ClaimManagerProvider); ok {
			...
			if err != nil {
				logger.WithIndent().Warningf("Failed to get claims for BSN %s: %v", bsn, err)
			} else {
