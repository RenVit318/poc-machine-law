...
func (cm *ClaimManager) ApproveClaim(claimID, verifiedBy string, verifiedValue interface{}) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
...
func (cm *ClaimManager) RejectClaim(claimID, rejectedBy, rejectionReason string) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
